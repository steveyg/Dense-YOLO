...
parser.add_argument('--predicted_file', default='predicted_file_level1', type=str, help='Checkpoint state_dict file for transfer learning')
args = parser.parse_args()

annopath = r'D:/buaa/机器学习/测试脚本/Anno_test/%s.xml'#测试集标注文件路径
test_imagesetfile = 'D:/buaa/机器学习/测试脚本/core_coreless_test.txt'#测试集里面的图片名
'''
...
def parse_rec(filename):
    ...
    objects = []
    # 还需要同时打开图像，读入图像大小
    # img = cv2.imdecode(np.fromfile(imagename1,dtype=np.uint8),-1)
    # if img is None:
    #     img = cv2.imdecode(np.fromfile(imagename2,dtype=np.uint8),-1)
    # if img is None:
    img = cv2.imdecode(np.fromfile(imagename3,dtype=np.uint8),-1)
